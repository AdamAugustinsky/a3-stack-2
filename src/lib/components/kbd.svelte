<script lang="ts">
    ...
    const defaultClass = 'border-neutral-200/60 bg-background/80 text-muted-foreground dark:border-neutral-700/50';
    const onPrimaryClass = 'bg-primary-foreground/20 text-primary-foreground border-primary-foreground/30';
    $effect(() => {
...
{#if content}
    ...
    <kbd
        class={cn(
            baseClass,
            variant === 'onPrimary' ? onPrimaryClass : defaultClass,
            className
        )}
        {...restProps}
    >
